...
namespace GPsoftware.Web.Optimization.Test {
    ...
    public class BundleCollectionTest {
        ...
        public void SameBundlePathReplacesTests() {
            BundleCollection col = new BundleCollection();
            DynamicFolderBundle db1 = new DynamicFolderBundle("foo", "*.js", JsMinifyUglify.Instance);
            col.Add(db1);
            Assert.AreEqual(1, col.Count);
            DynamicFolderBundle db2 = new DynamicFolderBundle("foo", "*.css", CssMinifyUglify.Instance);
            col.Add(db2);
            Assert.AreEqual(1, col.Count);
